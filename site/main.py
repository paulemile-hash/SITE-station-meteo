...
def index():
    ...
    moyenne_humidite = cur.execute(
        "SELECT AVG(humidite) FROM mesures").fetchone()[0]
    moyenne_temperature = cur.execute(
        "SELECT AVG(temperature) FROM mesures").fetchone()[0]
    moyenne_force_vent = cur.execute(
        "SELECT AVG(force_vent) FROM mesures").fetchone()[0]
...
def temperature():
    ...
    data_temperature = cur.execute(
        "SELECT time,temperature FROM mesures").fetchall()
...
def vent():
    ...
    data_vent = cur.execute(
        "SELECT time,force_vent, direction_vent FROM mesures").fetchall()
